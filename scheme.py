...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        arguments_list = []
        # BEGIN PROBLEM 3
        def helper(scheme_lst):
            if scheme_lst is nil:
                return scheme_lst
            arguments_list.append(scheme_lst.first)
            helper(scheme_lst.rest)
        helper(args)
        if self.use_env:
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        if not (scheme_eval(expressions.first, env) is False):
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    first_element = scheme_eval(expressions.first, env)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        validate_form(read_line(str(bindings.first)), 2, 2)
        names, values, bindings = Pair(bindings.first.first, names), Pair(scheme_eval(bindings.first.rest.first, env), values), bindings.rest
